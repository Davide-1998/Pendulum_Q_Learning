...
class Pendulum:
    ...
    * visuals: the list of all the 'visual' 3D objects to render the robot,
    ...
    def __init__(self, nbJoint=1, noise_stddev=0.0, coefficient=1):
...
#    def nobs(self): return self.nx+self.withSinCos
    ...
    def dynamics(self, x, u, display=False):
        ...
        for i in range(self.NDT):
            ...
            M = self.data.M
            # this "b" term is the problem
            # self.coefficient has being added in order to show its effect
            b = self.data.nle * self.coefficient
            inv_M = inv(M)
            z = (u-self.Kf*v-b)
            a = inv_M.dot(z)
